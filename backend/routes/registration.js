...
router.put('/user/:userId', async (req, res) => {
  try {
    ...
    if (email && email !== user.email) {
      const existingUser = await Registration.findOne({ email: email.trim().toLowerCase() });
      if (existingUser) {
        return res.status(409).json({
          success: false,
    ...
    if (registerId !== undefined) user.registerId = registerId;

    await user.save();

    // Return updated user without password
    const updatedUser = await Registration.findOne({ userId }).select('-password');
